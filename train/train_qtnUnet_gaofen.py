#!/usr/bin/env python
"""
QTNUnet 训练脚本 (Gaofen 数据集, level0, 单尺度)
基于 train_mecnet_gaofen.py 修改
"""
import os
import sys
import argparse
...
from SPP_model.modeling.other_model.QTNUnet.qtnUnet import Encoder, Decoder, SegmentationHead


class QTNUnetFixed(nn.Module):
    """修正了 decoder embed_dims 通道数不匹配问题的 QTNUnet"""
    def __init__(self, img_size=1024, in_chans=3,
                 embed_dims=[16, 64, 128, 256, 256],
                 mlp_ratios=[4, 4, 4, 4, 4],
                 norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 depths=[3, 3, 5, 2, 2]):
        super().__init__()
        self.encoder = Encoder(img_size=img_size, in_chans=in_chans,
                               embed_dims=embed_dims, mlp_ratios=mlp_ratios,
                               norm_layer=norm_layer, depths=depths, num_stages=5)
        self.decoder = Decoder(img_size=32, embed_dims=[256, 128, 64, 16],
                               mlp_ratios=mlp_ratios, norm_layer=norm_layer,
                               depths=depths, num_stages=4, stride=[2, 2, 2, 2])
        self.segmentationHead = SegmentationHead(num_classes=1, inchannels=16)
...
def main():
    ...
    parser.add_argument("--task_name", type=str, default="Golden_QTNUnet")
    ...
    logger.info("=" * 60)
    logger.info("Training QTNUnet on Golden (level0)")
    logger.info("=" * 60)
    ...
    # 加载 QTNUnet (修正版)
    model = QTNUnetFixed(img_size=1024, in_chans=3, embed_dims=[16, 64, 128, 256, 256],
                         mlp_ratios=[4, 4, 4, 4, 4], norm_layer=partial(nn.LayerNorm, eps=1e-6),
                         depths=[3, 3, 5, 2, 2])
