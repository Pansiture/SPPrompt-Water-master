#!/usr/bin/env python
"""
SPPrompt-Water + EDL 训练脚本 (Gaofen_processed_v2 数据集, 多尺度 ConcatDataset)
基于 train_edl.py 修改，适配 Gaofen 数据格式，三尺度 level0/1/2 同时训练。
"""
import logging
import sys
import argparse
...
class PromptDataset_Gaofen(Dataset):
    """适配 Gaofen_processed_v2 (png 格式) 的 PromptDataset。"""
...
parser.add_argument("--task_name", type=str, default="SPP_Gaofen_EDL_MultiScale")
...
logger.info("=" * 60)
logger.info("Training Configuration (EDL Version - Gaofen MultiScale):")
...
def validate(prompt_water_net, val_dataloader, device, epoch, num_epochs,
    ...
    with torch.no_grad():
        for batch in tqdm(val_dataloader, desc="Validation"):
            if len(batch) == 4:
                # 训练模式: (image, gt, prompt, name)
                image, labels, prompts, _ = batch
            elif len(batch) == 3:
                # 推理模式: (image, prompt, gt_path) -> 需要重新加载 gt
                image, prompts, gt_paths = batch
                # 从路径加载 gt
                labels = []
                for p in gt_paths:
                    gt = io.imread(p)
                    ...
                    if gt.max() > 1.0:
                        gt = gt / 255.0
                    labels.append(torch.from_numpy(gt).float())
...
def main():
    ...
    train_datasets = []
    for lv in levels:
        ds_path = args.data_train.replace("level0", lv)
        if os.path.isdir(ds_path):
            ds = PromptDataset_Gaofen(ds_path)
            train_datasets.append(ds)
            logger.info("  %s train samples: %d", lv, len(ds))
        else:
            logger.warning("  %s path not found: %s", lv, ds_path)
    train_dataset = ConcatDataset(train_datasets)
    logger.info("Total training samples (ConcatDataset): %d", len(train_dataset))

    val_datasets = []
    for lv in levels:
        val_path = (args.data_val if args.data_val else args.data_train.replace("train", "val")).replace("level0", lv)
        if os.path.isdir(val_path):
            ds = PromptDataset_Gaofen(val_path, inference=True)
            val_datasets.append(ds)
            logger.info("  %s val samples: %d", lv, len(ds))
        else:
            logger.warning("  %s val path not found: %s", lv, val_path)
    val_dataset = ConcatDataset(val_datasets)
    logger.info("Total val samples (ConcatDataset): %d", len(val_dataset))
