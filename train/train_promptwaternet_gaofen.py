#!/usr/bin/env python
"""
Train SPPrompt-WaterNet on Gaofen_processed_v2 dataset.
Adapted from train_promptwaternet.py with Gaofen-specific paths and settings.
"""
import logging
import sys
import argparse
...
from RSDataloader import PromptDataset_GID5, MultiScaleGaofenDataset
...
parser.add_argument("--data_train", type=str, default="/root/autodl-tmp/SPPrompt-Water-master/data/Gaofen_processed_v2/level0/train",
...
parser.add_argument("--promptcp", type=str, default="/root/autodl-tmp/SPPrompt-Water-master/pretrain/sam_vit_b_01ec64.pth",
...
parser.add_argument("--task_name", type=str, default="SPP_Gaofen_v2")
...
def main():
    ...
    logger.info("Number of image encoder and mask decoder parameters: %s",
    ...
    # ---- Multi-level dataset (ConcatDataset mode, same as GID) ----
    # Each batch contains images from ONE level, not all three simultaneously
    levels = ["level0", "level1", "level2"]
    train_datasets = []
    for lv in levels:
        ds = PromptDataset_GID5(args.data_train.replace("level0", lv), inference=False)
        train_datasets.append(ds)
    train_dataset = ConcatDataset(train_datasets)

    val_datasets = []
    for lv in levels:
        ds = PromptDataset_GID5(args.data_train.replace("train", "val").replace("level0", lv), inference=False)
    ...
    logger.info("Dataset Configuration (ConcatDataset Mode):")
    for i, lv in enumerate(levels):
        logger.info("  %s train: %d, val: %d", lv, len(train_datasets[i]), len(val_datasets[i]))
    logger.info("  TOTAL Train: %d, Val: %d", len(train_dataset), len(val_dataset))
    logger.info("  Note: each batch from ONE level, same as GID training")
    logger.info("=" * 60)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        logger.info('Time: %s, Epoch: %d, Loss: %.4f, LR: %.6f', 
                    datetime.now().strftime("%Y%m%d-%H%M"), epoch, epoch_loss, optimizer.param_groups[0]['lr'])
