#!/usr/bin/env python
"""
SwinTransformer 训练脚本 (Gaofen 数据集, level0)
基于 train_unet_gaofen.py 简化，固定 model_type=swin，适配 1024x1024 输入。
"""
import os
import sys
import argparse
...
def main():
    ...
    parser.add_argument("--task_name", type=str, default="Swin_Gaofen")
    ...
    model_save_path = os.path.join(args.work_dir, f"{args.task_name}_swin-{run_id}")
    ...
    logger.info("=" * 60)
    logger.info("Training SwinTransformer on Gaofen (level0)")
    logger.info("=" * 60)
