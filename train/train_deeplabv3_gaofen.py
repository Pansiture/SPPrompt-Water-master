#!/usr/bin/env python
"""
DeepLabV3+ (ResNet-101) 训练脚本 (Gaofen 数据集, level0)
基于 train_baseline.py 修改，使用 torchvision.models.segmentation.deeplabv3_resnet101
"""
import os
import sys
import argparse
...
def validate(model, val_dataloader, device, loss_fn):
    ...
    with torch.no_grad():
        for image, labels, _ in tqdm(val_dataloader, desc="Validation"):
            ...
            pred = model(image)
            # torchvision 的 segmentation 模型输出 dict
...
def main():
    ...
    parser.add_argument("--task_name", type=str, default="DeepLabV3Plus_Golden")
    ...
    logger.info("=" * 60)
    logger.info("Training DeepLabV3+ (ResNet-101) on Golden (level0)")
    logger.info("=" * 60)
    ...
    # 加载 DeepLabV3+ (ResNet-101 backbone)
    # torchvision 默认输出 21 类 (VOC)，需要替换为 1 类
