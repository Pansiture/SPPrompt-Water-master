...
# -*- coding: utf-8 -*-
"""
Training script for MSResNet baseline on Gaofen dataset (level0, single-scale).
修复: 关闭 AMP, 添加梯度裁剪, 降低 lr 避免 NaN
"""
import argparse
import logging
import sys
...
def main():
    ...
    parser.add_argument("--task_name", type=str, default="MSResNet_Gaofen")
    ...
    parser.add_argument("--use_amp", action="store_true", default=False, help="Enable AMP (may cause NaN with MSResNet)")
    ...
    # ---- model ----
    pretrained_path = "/root/autodl-tmp/SPPrompt-Water-master/pretrain/resnet34-b627a593.pth"
    model = MSResNet(in_channels=3, num_classes=1, pretrained_path=pretrained_path).to(device)
    model.train()
    ...
    if os.path.isfile(pretrained_path):
        logger.info("Loaded pretrained ResNet34 from: %s", pretrained_path)
    else:
        logger.info("Pretrained path not found: %s", pretrained_path)
    ...
    if args.use_amp:
        ...
        logger.info("AMP enabled (GradScaler). WARNING: May cause NaN with MSResNet.")
    else:
