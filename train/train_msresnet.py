...
# -*- coding: utf-8 -*-
"""
Training script for MSResNet baseline
...
def validate(model, val_dataloader, device, loss_fn):
    ...
    with torch.no_grad():
        for image, labels, _, _ in tqdm(val_dataloader, desc="Validation"):
            ...
            # normalize image if in [0,255]
            if image.max() > 1.0:
                image = image / 255.0
            pred = model(image)
            # align spatial size with labels
...
def main():
    ...
    parser.add_argument("--task_name", type=str, default="MSResNet_GLH",
    ...
    parser.add_argument("--pretrained_path", type=str,
                        ...
                        help="Path to local ResNet34 pretrained weights")
    ...
    # ---- model ----
    pretrained_arg = args.pretrained_path if os.path.isfile(args.pretrained_path) else False
    model = MSResNet(in_channels=3, num_classes=1, pretrained=pretrained_arg).to(device)
    model.train()
    ...
    # sanity check first sample shape
