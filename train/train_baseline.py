#!/usr/bin/env python
"""
Baseline (UNet / SwinTransformer) 训练脚本
支持：纯 UNet 或纯 SwinTransformer 训练，计算 mIoU/Acc/F1，保存最佳模型和日志。
"""
import os
import sys
import argparse
...
class SimpleDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        gt = np.expand_dims(gt, axis=0).astype(np.float32) / 255.0
...
def validate(model, val_dataloader, device, loss_fn, model_type):
    model.eval()
    val_loss = 0
...
def main():
    ...
    parser.add_argument("--swin_pretrained", type=str, default="/root/autodl-tmp/SPPrompt-Water-master/pretrain/swin_tiny_patch4_window7_224.pth")
    ...
    parser.add_argument("--input_size", type=int, default=0, help="Resize input for UNet (0 for no resize)")
    args = parser.parse_args()

    run_id = datetime.now().strftime("%Y%m%d-%H%M")
    model_save_path = os.path.join(args.work_dir, f"{args.task_name}_{args.model_type}-{run_id}")
    ...
    logger.info("=" * 60)
    logger.info("Training Baseline (%s)", args.model_type.upper())
    logger.info("=" * 60)
    ...
    logger.info("Input size: %s", args.input_size if args.input_size > 0 else "original")
    logger.info("=" * 60)

    if args.model_type == "unet":
        model = UNet(in_channels=3, out_channels=1)
    else:
        model = SwinTransformerNet(pretrained=args.swin_pretrained)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        val_loss, val_miou, val_acc, val_f1 = validate(model, val_loader, device, combined_loss, args.model_type)
