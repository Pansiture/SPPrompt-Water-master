...
#copyright:@article{Alisirs2021MECNet,
...
#  year={2021}
#}
#original link:https://github.com/Alisirs/MECNet
...
#         self.bn1 = nn.BatchNorm1d(inter_channels)  # 改为 BatchNorm1d
#         self.relu = nn.ReLU(inplace=True)
#         self.fc2 = nn.Conv2d(inter_channels, channels, 1)
#         self.bn2 = nn.BatchNorm1d(channels)  # 改为 BatchNorm1d
#         self.sigmoid = nn.Sigmoid()
#
#     def forward(self, x):
#         w = F.adaptive_avg_pool2d(x, 1)  # 输出形状: [batch, C, 1, 1]
#         w = self.fc1(w)
#         w = w.view(w.shape[0], w.shape[1])  # 变成 [batch_size, num_channels] 适配 BatchNorm1d
#         w = self.bn1(w)
#         w = self.relu(w)
#         w = self.fc2(w)
#         w = w.view(w.shape[0], w.shape[1])  # 适配 BatchNorm1d
#         w = self.bn2(w)
#         w = self.sigmoid(w)
#         w = w.view(w.shape[0], w.shape[1], 1, 1)  # 变回 [batch_size, channels, 1, 1] 进行广播
#         x = w * x + x
#         return x
...
class GlobalFieldConv2d(nn.Module):
    def __init__(self, in_channels, channels, reduction_factor=4):
        ...
        inter_channels = max(in_channels // reduction_factor, 32)
        self.fc1 = nn.Conv2d(in_channels, inter_channels, 1)
        self.bn1 = LayerNorm2d(inter_channels)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.Conv2d(inter_channels, channels, 1)
        self.bn2 = LayerNorm2d(channels)
        self.sigmoid = nn.Sigmoid()
...
if __name__ == '__main__':
    ...
    img = torch.rand(2, 4, 448, 448)  # 128 448 256 512
    net = MECNet()
    net = net.to(device)
    img = img.to(device)
    output = net(img)
