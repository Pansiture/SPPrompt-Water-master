...
class Encoder(nn.Module):
    def __init__(self, img_size=256, in_chans=3, depths=[3, 4, 6, 3], num_stages=4, drop_rate=0., drop_path_rate=0.,
                 stride=[2, 2, 2, 2,2], mlp_ratios=[4, 4, 4, 4], embed_dims=[64, 128, 256, 512],
...
class qtnUnet(nn.Module):
    def __init__(self,img_size=256, in_chans=3, embed_dims=[16, 64, 128, 256], mlp_ratios=[4, 4, 4, 4],
                norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 3, 5, 2],):
        super(qtnUnet, self).__init__()
        self.encoder = Encoder(img_size=img_size, in_chans=in_chans, embed_dims=embed_dims, mlp_ratios=mlp_ratios,
                norm_layer=norm_layer, depths=depths,num_stages=5)
        self.decoder = Decoder(img_size=32, embed_dims=[ 128, 64, 32,16], mlp_ratios=mlp_ratios,
                norm_layer=norm_layer, depths=depths,num_stages=4,stride=[2, 2, 2, 2])
