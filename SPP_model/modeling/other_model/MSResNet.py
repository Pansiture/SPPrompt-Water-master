# MSResNet: Multiscale Residual Network via Self-Supervised Learning for
# Water-Body Detection in Remote Sensing Imagery
...
class MSDC(nn.Module):
    ...
    def forward(self, feats):
        ...
        fusion = [F.interpolate(input=stage(feats), size=(h, w), mode='bilinear') for stage in self.stages] + [feats]
...
class MKMP(nn.Module):
    ...
    def forward(self, x):
        ...
        self.sub1 = F.interpolate(self.subpart1, size=(height, width), mode='bilinear', align_corners=False)
        
        self.subpart2 = self.conv2d(self.maxpool2(x))
        self.sub2 = F.interpolate(self.subpart2, size=(height, width), mode='bilinear', align_corners=False)
        
        self.subpart3 = self.conv2d(self.maxpool3(x))
        self.sub3 = F.interpolate(self.subpart3, size=(height, width), mode='bilinear', align_corners=False)
        
        self.subpart4 = self.conv2d(self.maxpool4(x))
        self.sub4 = F.interpolate(self.subpart4, size=(height, width), mode='bilinear', align_corners=False)
