...
def edl_mse_loss(alpha, target, K=2, pos_weight=10.0):
    """
    EDL MSE loss for segmentation with class imbalance handling.
    Args:
        alpha: (B, K, H, W) Dirichlet parameters
        target: (B, 1, H, W) with values near 0 or 1
        pos_weight: weight for foreground pixels to combat class imbalance
    Returns:
        loss: scalar
    """
    # Defensive: normalize if masks are 0/255
    if target.max() > 1.0:
        target = target / 255.0
    ...
    # Robust one-hot encoding (tolerant to float imprecision after interpolation)
    y = torch.zeros_like(alpha)
    y[:, 0:1, :, :] = (target < 0.5).float()
    y[:, 1:2, :, :] = (target > 0.5).float()
    ...
    per_pixel = mse + var  # (B, 1, H, W)

    # Class weighting: foreground pixels get pos_weight to combat imbalance
...
def edl_loss(evidence, target, epoch_num, num_epochs, K=2, kl_anneal_epochs_ratio=0.5, kl_scale=0.5, pos_weight=10.0):
    """
    ...
    Args:
        ...
        pos_weight: foreground weight for class imbalance (default 10.0)
    Returns:
        total_loss: scalar
        mse_loss: scalar
        kl_loss: scalar
    ...
    mse = edl_mse_loss(alpha, target, K=K, pos_weight=pos_weight)
