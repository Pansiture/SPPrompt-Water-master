...
def main():
    ...
    for img_path in tqdm(img_list, desc="Evaluating"):
        ...
        gt_tensor = torch.tensor(gt).float().unsqueeze(0)

        # Calculate per-image metrics
        img_miou, img_acc, img_f1 = calculate_metrics(pred_resized, gt_tensor)
        ...
        # Move to CPU to avoid OOM when accumulating thousands of images
        all_preds.append(pred_resized.cpu())
        all_gts.append(gt_tensor.cpu())
        del pred, pred_resized, gt_tensor
