...
def run_pyramid_batched(images, prompts, mode='real'):
    """Batched pyramid inference. images: (B, C, H, W), prompts: (B, 1, H, W) or None"""
...
def run_batched_inference(prompt_mode='real'):
    ...
    print(f"\n[Run] Prompt mode: {prompt_mode.upper()}, images: {len(dataset)}, batch_size: {BATCH_SIZE}")
    
    for images, prompts, gts, basenames in tqdm(dataloader, desc=prompt_mode):
        if prompt_mode == 'zero':
            preds = run_pyramid_batched(images, prompts, mode='zero')
        else:
            preds = run_pyramid_batched(images, prompts, mode='real')
