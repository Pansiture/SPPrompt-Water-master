...
def run_pyramid_with_real_prompt(image, real_prompt):
    """3层金字塔推理，顶层用真实prompt，下层用上层输出"""
...
for i, img_path in enumerate(img_files):
    ...
    pred = run_pyramid_with_real_prompt(img, prompt)
...
print(f'Mean mIoU (Pyramid + Real Prompt): {np.mean(miou_list):.4f}')
